...
def key_creator(
    ...
    if key_names is None or len(key_names) != 2:
        key_names = ["public_key.pem", "private_key.pem"]

    # Write the key bytes to a file
    with open(f"{key_names[0]}", "wb") as file:
        file.write(public_key_bytes)
    with open(f"{key_names[1]}", "wb") as file:
        ...
        with semaphore:  # acquire and release the semaphore
            try:
                ...
                (
                 ...
                 if current_thread in threads_set
...
class _FernetMethods:
    ...
    @staticmethod
    def _verify_peppers(
            hash_pepper: bytes | str,
            password_pepper: bytes | str
    ):
        match hash_pepper:
            case bytes():
                pass
            ...
            case _:
                raise TypeError(
                    'Hash pepper must be bytes or str'
                )
        ...
        match password_pepper:
            ...
            case _:
                raise TypeError(
                    'Password pepper must be bytes or str'
                )
    ...
    def encrypt_file(
        ...
        prep_types = self._verify_peppers(
            hash_pepper, password_pepper
        )
        hash_pepper = prep_types.get('hash_pepper')
        password_pepper = prep_types.get('password_pepper')
...
class _RSAMethods:
    ...
    def generate_keys(
        ...
        Quick help:
            ...
            - {filename | "public_key.pem"} [public_key]
            - {filename | "private_key.pem"} [private_key]

            Number refers to a random number between 1 and 65537.

        :param key_names: [list | set | tuple | None, default: None]
